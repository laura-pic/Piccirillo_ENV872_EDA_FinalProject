...
# retry plot after outlier set to NA
seIV_plot_again <- ggplot(selenium_conc, aes(Se_IV_D_CONC_BOTTLE, depth)) + geom_point() +
  scale_y_reverse(limits = c(6000, 0)) + 
  labs(y = "Depth (m)", x = "SeIV Concentration (nm/L)") + 
  geom_smooth(method = lm, se = F)

seVI_plot <- ggplot(selenium_conc, aes(Se_VI_D, depth)) + geom_point() + 
...
# remove zeroes? data can never truly be 0

# comparing frequency of continuous variables (concentrations) 
ggplot(selenium_conc) + geom_freqpoly(aes(x = Se_IV_D_CONC_BOTTLE)) + 
  geom_freqpoly(aes(x = Se_VI_D), color = "blue")
