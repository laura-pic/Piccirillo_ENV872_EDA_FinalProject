...
ggplot(OMZ_only, aes(Sbeox0, depth)) + geom_point() + scale_y_reverse()

ggplot(OMZ_only, aes(Org_Se_II_CONC_BOTTLE, depth)) + geom_point() + scale_y_reverse() + geom_smooth(method = lm, se = F)
...
```{r}
# plot cruise track locations of Se data with OMZ locations analyzed in a different color
```
