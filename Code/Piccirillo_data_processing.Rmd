...
# choose locations of interest - picked points with nearly complete depth profiles of oxygen and filtered them by lat/long - picked ones closest to Peruvian coast as this is where OMZ is
oxygen_location1 <- oxygen_clean2 %>%
  filter(LATITUDE == -12.00517 & LONGITUDE == -79.19517)

oxygen_location2 <- oxygen_clean2 %>%
  filter(LATITUDE == -12.04483 & LONGITUDE == -77.818)

oxygen_location3 <- oxygen_clean2 %>%
  filter(LATITUDE == -11.99983 & LONGITUDE == -81.50017)

# checking oxygen depth profiles to confirm locations are in OMZ
ggplot(oxygen_location1, aes(Sbeox0, DepSM)) + geom_point() + scale_y_reverse()
ggplot(oxygen_location2, aes(Sbeox0, DepSM)) + geom_point() + scale_y_reverse()
ggplot(oxygen_location3, aes(Sbeox0, DepSM)) + geom_point() + scale_y_reverse()
