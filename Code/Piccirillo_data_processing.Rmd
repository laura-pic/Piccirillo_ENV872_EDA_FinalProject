...
ggplot(oxygen_location2, aes(Sbeox0, DepSM)) + geom_point() + 
  scale_y_reverse() + labs(x = "Dissolved Oxygen (umol/kg)", y = "Depth (m)")
ggplot(oxygen_location3, aes(Sbeox0, DepSM)) + geom_point() + 
  scale_y_reverse() + labs(x = "Dissolved Oxygen (umol/kg)", y = "Depth (m)")
...
all_clean <- left_join(
  ...
  join_by(EVENT_LAT == LATITUDE, EVENT_LON == LONGITUDE))
# this didn't work because none of the lat/long matched from the oxygen dataset
```
